...
# Let GPT output be parsed as JSON by itself
try:
    data = eval(raw)  # safe here because GPT is constrained to JSON-like output
except Exception:
    logger.error("GPT fallback returned malformed JSON.")
    raise RuntimeError("GPT fallback failed: invalid JSON")

# Normalize into EventRead schema
fight_pairs = [
    FightPair(
